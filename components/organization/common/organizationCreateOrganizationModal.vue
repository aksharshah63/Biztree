<template>
  <bib-modal-wrapper v-if="show" size="lg" @close="$emit('close')">
    <div slot="content" class="pl-1 pr-1">
      <div class="font-w-700 pb-2">{{ $t("common.createOrganization") }}</div>
      <bib-input type="text" v-model="user_email" :label="`${$t('common.organizationName')} *`" placeholder="..." required></bib-input>

      <div class="mt-2 mb-2">
        <bib-input class="mb-05" type="text" v-model="user_email" :label="$t('common.inviteTeamMembers')" :placeholder="$t('common.enterNameOrEmail')"></bib-input>
    ...
    <div class="align-center justify-between p-1" slot="footer">
      ...
      <bib-button size="lg" :label="$t('common.create')" variant="primary-24" pill></bib-button>
    </div>
  </bib-modal-wrapper>
</template>
  ...
  data() {
    return {
      ...
      selectedTeammates: [
        ...
        { email: "brought.dust@gmail.coma", name: "Brought Dust" },
        { email: "john.doe@gmail.comb", name: "John Doe" },
        { email: "drinking.often@gmail.comb", name: "Drinking Often" },
        { email: "brought.dust@gmail.comb", name: "Brought Dust" },
      ],
    };
  },
  methods: {
